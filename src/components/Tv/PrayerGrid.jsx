...
const PrayerGrid = memo(function PrayerGrid({ listaNamazeve, vaktiSot, infoTani, xhematiFn, ne24hFn, isRamazan }) {
    // Only add the bottom spacer row when Sabahu's Lindja sub-card exists.
    // This keeps all times at the same vertical position across all cards.
    const anyHasSubCard = !isRamazan && !!vaktiSot.Lindja;
            ...
            <div className="bg-zinc-900 rounded-[3.5rem] p-6 border border-white/5 shadow-premium h-full flex flex-col justify-center">
                <div className={`grid gap-4 h-full ${listaNamazeve.length === 5 ? 'grid-cols-5' : 'grid-cols-6'}`}>
                    {listaNamazeve.map(({ id, label }) => {
                        ...
                        const hasLindja = id === 'Sabahu' && anyHasSubCard;

                        return (
                            <div
                                key={id}
                                className={`flex flex-col rounded-[3rem] px-2 py-4 items-center justify-between border-2 transition-[background-color,border-color,transform,box-shadow] duration-500 relative overflow-hidden ${isCurrent ? 'bg-emerald-600 border-white/60 z-10 shadow-[0_0_50px_rgba(16,185,129,0.3)]' : isNext ? 'bg-zinc-800/80 border-emerald-500/50' : isJumuah ? 'bg-amber-900/20 border-amber-500/30' : 'bg-black/40 border-white/5'}`}
                            >
                                {/* Top: prayer name — always-reserved subtitle slot prevents height jumps */}
                                ...
                                {/* Middle: time — always centered in remaining space */}
                                ...
                                {/* Bottom: Lindja sub-card OR invisible spacer for consistent alignment */}
                                {anyHasSubCard && (
                                    <div className="w-full px-2">
                                        {hasLindja ? (
                                            ...
                                            <div className="invisible py-4 rounded-3xl flex flex-col items-center">
